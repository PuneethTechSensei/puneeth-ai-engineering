(function(){
  ...
  const renderLab=(data,esc)=>{const x=data.experience||{};const saved=readLabEvidence(data.id);const checks=saved?.checks||{};return `<section class="lab-shell" id="engineeringLab"><div class="lab-head"><div><div class="experience-kicker">ENGINEERING LAB · ${x.type||'BUILD'}</div><h2>${x.title||'Engineering Mission'}</h2><p>${x.task||''}</p></div><div class="lab-time"><span>Suggested time</span><b>${x.time||'30–45 min'}</b></div></div><div class="lab-grid"><div class="lab-panel"><div class="lab-label">YOUR MISSION</div><ol class="lab-steps">${labGuidance(x.type).map(g=>`<li>${g}</li>`).join('')}</ol><div class="evidence"><b>Required evidence</b><span>${x.evidence||'Show the result and explain the engineering decision.'}</span></div><div class="lab-break"><div class="lab-label">BREAK IT</div><b>${x.breakIt||'Introduce one realistic failure and diagnose it.'}</b></div></div><div class="lab-panel lab-submit"><div class="lab-label">PROOF OF WORK</div><label class="lab-check"><input type="checkbox" data-lab-check="reproduce" ${checks.reproduce?'checked':''}> I can reproduce the result.</label><label class="lab-check"><input type="checkbox" data-lab-check="explain" ${checks.explain?'checked':''}> I can explain why it works.</label><label class="lab-check"><input type="checkbox" data-lab-check="diagnose" ${checks.diagnose?'checked':''}> I can diagnose the failure case.</label><label class="lab-field"><span>What did you actually observe?</span><textarea id="labEvidence" rows="7" placeholder="Record your output, metric, calculation, screenshot description, or root-cause note.">${saved?.text?esc(saved.text):''}</textarea></label><button class="btn primary" id="saveLab">${saved?'✓ Evidence saved':'Save lab evidence'}</button><div id="labStatus" class="lab-status">${saved?'Saved locally. You can now complete the lesson.':'Completion requires evidence, not just reading.'}</div></div></div></section>`};
  ...
  if(lesson){
    ...
    else {const esc=s=>String(s).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;');const resources=(data.resources||[]).map(r=>`<li><a href="${r.url}" target="_blank" rel="noopener">${r.label} ↗</a></li>`).join('');const saved=readLabEvidence(data.id);const needsLab=!!data.experience;lesson.innerHTML=`<div class="eyebrow">PHASE ${data.phase} · ${data.phaseName.toUpperCase()} ${preview?'· PREVIEW':''}</div><h1>${data.title}</h1><div class="meta">LESSON ${data.id} · LEARN / EXPERIMENT / BUILD / BREAK / PROVE</div>${preview&&!unlocked?'<div class="callout warning">Preview mode: this lesson is visible so you can see what is ahead. Complete the previous phase to unlock progress tracking.</div>':''}<p class="lead">${data.summary}</p><div class="callout"><b>Why this matters</b><p>${data.body}</p></div><h2>What you'll learn</h2><ul>${data.takeaways.map(x=>`<li>${x}</li>`).join('')}</ul><h2>Build it</h2><p>Start with the smallest useful implementation. Inspect intermediate values before adding abstractions.</p><pre><code>${esc(data.code)}</code></pre><div class="practice-brief"><div class="experience-kicker">HANDS-ON PRACTICE</div><h2>Do the work</h2><p>${data.practice}</p></div>${renderLab(data,esc,prog)}<h2>Common mistakes</h2><ul>${(data.mistakes||['Skipping the prerequisite concept.','Copying code without understanding the output.','Measuring only whether code runs, not whether it is correct.']).map(x=>`<li>${x}</li>`).join('')}</ul>${resources?`<h2>Further reading</h2><ul>${resources}</ul>`:''}${unlocked?`<div class="completion-block"><div class="completion-copy"><div class="experience-kicker">PROGRESS GATE</div><b>${saved?'Evidence recorded.':'Complete the lab before marking this lesson complete.'}</b><span>A lesson is complete when you have produced evidence and can explain the result.</span></div><button id="complete" class="complete ${prog[data.id]?'done':''}" ${needsLab&&!saved?'disabled':''}>${prog[data.id]?'✓ Completed':saved?'Mark lesson complete':'Save lab evidence first'}</button></div>`:'<div class="callout">🔒 Complete the prerequisite phase to mark this lesson complete.</div>'}`;
      if(unlocked){
        const saveBtn=document.getElementById('saveLab');
        saveBtn?.addEventListener('click',()=>{const checks={};document.querySelectorAll('[data-lab-check]').forEach(c=>checks[c.dataset.labCheck]=c.checked);const text=(document.getElementById('labEvidence')?.value||'').trim();const status=document.getElementById('labStatus');if(!checks.reproduce||!checks.explain||!checks.diagnose){status.textContent='Complete all three self-checks before saving evidence.';status.className='lab-status error';return}if(text.length<40){status.textContent='Add at least a short observation (40+ characters). Evidence should say what happened, not only that you finished.';status.className='lab-status error';return}saveLabEvidence(data.id,{checks,text,savedAt:new Date().toISOString()});status.textContent='✓ Evidence saved locally. Your lesson completion gate is now open.';status.className='lab-status success';const complete=document.getElementById('complete');if(complete){complete.disabled=false;complete.textContent=prog[data.id]?'✓ Completed':'Mark lesson complete'}});
